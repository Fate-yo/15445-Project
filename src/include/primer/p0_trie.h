 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (auto &it : key) {
      ...
      if (!node->get()->HasChild(it)) {
        node = node->get()->InsertChildNode(it, std::make_unique<TrieNode>(it));
      } else {
        node = node->get()->GetChildNode(it);
      }
    }
    auto end_node = node->get()->GetChildNode(*(--key.end()));
    ...
    node = node->get()->InsertChildNode(*(--key.end()), std::make_unique<TrieNode>(*(--key.end())));
  ...
  bool Remove(const std::string &key) {
    ...
    latch_.WLock();
    std::vector<std::pair<char, std::unique_ptr<TrieNode> *>> v;
    auto node = &root_;
    auto pre = key.begin();
    ...
    while (!v.empty()) {
      ...
      v.pop_back();
      auto nodes = e.first;
      auto nex = (*e.second)->GetChildNode(nodes);
      if (nex != nullptr && (*nex)->HasChildren()) {
        continue;
      }
      (*e.second)->RemoveChildNode(nodes);
    }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    for (auto &it : key) {
      cnt += 1;
      auto new_node = node->get()->GetChildNode(it);
      if (!new_node) {
        *success = false;
        break;
      }
      if (new_node->get()->IsEndNode() && cnt == key.size()) {
        auto e = dynamic_cast<TrieNodeWithValue<T> *>(new_node->get());
        if (!e) {
          *success = false;
          break;
        }
